...
with tab1:
    ...
    with left:
        ...
        rank_box = st.container(height=520)
        with rank_box:
            if len(work) == 0:
                ...
                for i, r in work.iterrows():
                    ...
                    with st.container(border=True):
                        c_img, c_info, c_score = st.columns([1.05, 3.5, 1.25])

                        with c_img:
                            png, img_err = mol_png(r["canonical_isomeric"], 190, 140)
                            if png:
                                st.image(png, width=130)
                            else:
    ...
    with right:
        ...
        if len(work) == 0:
            ...
            png, img_err = mol_png(r["canonical_isomeric"], 420, 300)
            if png:
