...
실행
    streamlit run app_scentlab_final_dark.py
...
<style>
  ...
  * {
  ...
  .stApp {
      background-image:
        linear-gradient(rgba(7,17,31,.76), rgba(7,17,31,.82)),
...
# - 과학 배경은 페이지 빈 공간에서만 보이도록 유지
# - 데이터/정보가 있는 패널은 완전 불투명
# ══════════════════════════════════════════════
st.markdown("""
<style>
  /* 정보 카드 전체: 배경 완전 차단 */
  div[data-testid="stVerticalBlockBorderWrapper"] {
      ...
      border: 1px solid #2d4663 !important;
      backdrop-filter: none !important;
      -webkit-backdrop-filter: none !important;
      box-shadow: 0 10px 24px rgba(0,0,0,.12) !important;
  }

  /* 카드 내부가 어떤 부모 배경도 물려받지 않도록 */
  div[data-testid="stVerticalBlockBorderWrapper"]
  ...
  .smiles-guide-tip {
      margin-top: 9px;
      padding-top: 8px;
      border-top: 1px solid rgba(83,120,158,.30);
      color: #7f95ac;
      font-size: .75rem;
  }
...
# ══════════════════════════════════════════════
# Opaque information panels + hover help
# ══════════════════════════════════════════════
st.markdown("""
<style>
  /* 정보가 들어간 Streamlit 컨테이너는 완전 불투명 */
  div[data-testid="stVerticalBlockBorderWrapper"] {
      ...
      background-image: none !important;
      opacity: 1 !important;
      border: 1px solid #2b425d !important;
      box-shadow: 0 8px 22px rgba(0,0,0,.14) !important;
      overflow: hidden;
  }

  /*
    Streamlit 버전에 따라 실제 내부 block이 별도 wrapper를 가지므로
    여러 단계까지 모두 같은 불투명 배경을 지정한다.
  */
  ...
  .help-tooltip .proposal {
      color: #ffba82;
      font-weight: 800;
  }

  /* 배지 자체에 마우스를 올려도 의미를 알 수 있게 */
  .status-badge-help {
      cursor: help;
  }
...
# ══════════════════════════════════════════════
# Final fixed-dark UI
# PC/브라우저 Light/Dark 설정과 무관하게 동일한 화면 유지
# ══════════════════════════════════════════════
st.markdown("""
<style>
  html {
  ...
  /* ── Light mode에서 위젯이 흰색으로 튀는 현상 방지 ── */
  div[data-baseweb="select"] > div,
  [data-testid="stTextInput"] input,
  [data-testid="stNumberInput"] input,
  [data-testid="stNumberInput"] button {
  ...
  [data-testid="stTextInput"] input:disabled {
      ...
      -webkit-text-fill-color: #c1d0df !important;
      opacity: 1 !important;
  }
  ...
  .howto-step {
      ...
      border-radius: 8px;
      background: #101f31;
      border: 1px solid rgba(65,100,140,.35);
      font-size: .78rem;
      line-height: 1.55;
      color: #aebfd1;
  }
  ...
  .howto-foot {
      margin-top: 10px;
      padding-top: 9px;
      border-top: 1px solid rgba(70,108,150,.30);
      color: #8297ad;
      font-size: .75rem;
      line-height: 1.5;
  }
  ...
  .perf-bars {
      ...
      background:
        repeating-linear-gradient(
          ...
          transparent 0,
          transparent 48px,
          rgba(83,115,151,.15) 49px,
          rgba(83,115,151,.15) 50px
        );
  }
  ...
  .perf-bar {
      ...
      background: linear-gradient(180deg, #4c9de8 0%, #236fb5 100%);
      border: 1px solid rgba(123,188,246,.20);
  }
...
# ══════════════════════════════════════════════
with tab1:
    # 사용법 — 발표/실사용 시 필요할 때만 펼쳐볼 수 있도록 토글
    show_howto = st.toggle("사용법 보기", value=False, key="show_howto")

    if show_howto:
        st.markdown(
            """
            <div class="howto-box">
              <div class="howto-title">대시보드 사용법</div>
              <div class="howto-grid">
                <div class="howto-step">
                  <b>① 목표 향취 선택</b><br>
                  FRUITY, FLORAL, WOODY 등 찾고 싶은 향취를 선택합니다.
                  표시 개수를 정하고, 필요하면 <b>미기록 후보만</b>을 체크합니다.
                </div>
                <div class="howto-step">
                  <b>② 후보 분자 확인</b><br>
                  모델 점수가 높은 순서대로 후보가 표시됩니다.
                  <b>DB 확인</b>은 원본 데이터에 해당 향취가 기록된 분자,
                  <b>모델 제안</b>은 미기록이지만 모델 점수가 높은 후보입니다.
                </div>
                <div class="howto-step">
                  <b>③ 상세 검토</b><br>
                  후보를 선택해 분자 구조, 적합도, MolWt·LogP·TPSA 등 물성을 확인합니다.
                  실제 적용 전에는 관능평가와 안전성·규제 검토가 필요합니다.
                </div>
              </div>
              <div class="howto-foot">
                다른 탭에서는 <b>SMILES → 향취 예측</b>으로 신규 분자의 예상 향취를 확인하고,
                <b>모델 성능</b>에서 41개 향취별 Test 성능과 신뢰도를 확인할 수 있습니다.
                적합도는 실제 확률이 아니라 후보 우선순위를 위한 상대 점수입니다.
              </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with left:
        st.markdown(
            ...
            <div class="title-help-row">
              <span class="title-text">후보 분자 Ranking</span>
              <span class="help-tip">?
                <span class="help-tooltip">
                  목표 향취에 대해 모델 점수가 높은 분자를 순서대로 보여줍니다.<br><br>
                  <span class="db">● DB 확인</span> :
                  원본 데이터베이스에 현재 목표 향취가 이미 기록된 분자입니다.<br>
                  <span class="proposal">● 모델 제안</span> :
                  원본 DB에는 현재 목표 향취가 기록되지 않았지만 모델이 높은 점수를 부여한 후보입니다.
                  원본은 분자당 향취가 최대 3개까지만 기록되어 있으므로
                  <b>미기록 = 실제로 향이 없음</b>을 의미하지 않습니다.<br><br>
                  <b>적합도</b>는 후보 우선순위를 위한 상대 점수이며 실제 존재 확률은 아닙니다.
                  최종 판단에는 관능평가가 필요합니다.
                </span>
              </span>
            </div>
        ...
        rank_box = st.container(height=610, border=True)
        with rank_box:
            if len(work) == 0:
                ...
                for i, r in work.iterrows():
                    ...
                    with st.container(border=True):
                        ...
                        with c_info:
                            ...
                            badge_help = (
                                "DB 확인: 원본 데이터베이스에 현재 목표 향취가 기록된 분자"
                                if r["기록됨"]
                                else "모델 제안: 원본 DB에는 현재 목표 향취가 기록되지 않았지만 모델 점수가 높은 후보"
                            )
                            st.markdown(
                                f"**{r['molecule_id']}** "
                                f"<span class='pill {badge_cls} status-badge-help' title='{badge_help}'>{badge_txt}</span>",
                                unsafe_allow_html=True,
                            )
        ...
        if len(work) == 0:
            ...
            with st.container(border=True):
                ...
                with c2:
                    ...
                    st.markdown(
                        "<span class='pill pill-green status-badge-help' title='DB 확인: 원본 데이터베이스에 현재 목표 향취가 기록된 분자'>DB 확인</span>"
                        if r["기록됨"]
                        else "<span class='pill pill-orange status-badge-help' title='모델 제안: 원본 DB에는 현재 목표 향취가 기록되지 않았지만 모델 점수가 높은 후보. 관능평가 확인 필요'>모델 제안</span>",
                        unsafe_allow_html=True,
                    )
...
with tab2:
    ...
    st.markdown(
        """
        <div class="smiles-guide">
          <div class="smiles-guide-title">SMILES 입력 가이드</div>
          <div class="smiles-guide-grid">
            <div class="smiles-guide-item"><code>C N O S</code> 원자 기호</div>
            <div class="smiles-guide-item"><code>=</code> 이중결합 · <code>#</code> 삼중결합</div>
            <div class="smiles-guide-item"><code>( )</code> 가지 구조 표현</div>
            <div class="smiles-guide-item"><code>1 2 ...</code> 고리 연결 번호</div>
            <div class="smiles-guide-item"><code>@ / @@</code> 원자의 입체배치</div>
            <div class="smiles-guide-item"><code>/ \\</code> 이중결합의 입체 방향 표기</div>
            <div class="smiles-guide-item"><code>[ ]</code> 전하·특수 원자 직접 표기</div>
            <div class="smiles-guide-item"><code>.</code> 서로 분리된 성분 표시</div>
          </div>
          <div class="smiles-guide-tip">
            예시: 에탄올 <code>CCO</code> · 벤젠 <code>c1ccccc1</code> ·
            가지 구조 <code>CC(C)O</code> · 직접 작성이 어렵다면 PubChem 등에서 SMILES를 복사해 붙여넣는 방식을 권장합니다.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with tab3:
    ...
    # Streamlit 기본 차트는 OS/브라우저 테마에 따라 흰 배경이 나타날 수 있어
    # 테마 독립적인 HTML/CSS 다크 차트로 렌더링한다.
    chart_df = show.sort_values("label").copy()
    chart_max = max(float(chart_df["P@10"].max()), 1.0)

    bars = []
    for _, rr in chart_df.iterrows():
        val = float(rr["P@10"])
        bar_h = max(2, int((val / chart_max) * 150))
        safe_label = str(rr["label"]).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        bars.append(
            f"""
            <div class="perf-item" title="{safe_label} · P@10 {val:.2f}">
              <div class="perf-value">{val:.1f}</div>
              <div class="perf-bar" style="height:{bar_h}px;"></div>
              <div class="perf-label">{safe_label}</div>
            </div>
            """
        )
    ...
    st.markdown(
        ...
        <div class="perf-chart">
          <div class="perf-chart-title">향취별 Precision@10</div>
          <div class="perf-bars">{''.join(bars)}</div>
          <div class="perf-axis-note">
            P@10 = 목표 향취를 검색했을 때 상위 10개 후보 중 원본 데이터에 실제 기록된 정답의 비율
          </div>
        </div>
        """,
        unsafe_allow_html=True,
